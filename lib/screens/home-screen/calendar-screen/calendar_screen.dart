...
import '../category-screen/transaction_dispayer.dart';
      ...
      child: Container(
        ...
        child: Center(
          child: Padding(
            ...
            child: _showCalendar ? _buildCalendar() : _buildBudgetList(),
          ),
        ),
      ),
    );
  ...
  Widget _buildCalendar() {
    return TableCalendar(
      ...
      onDaySelected: (selectedDay, focusedDay) {
        setState(() {
          _showCalendar = false;
          _selectedDay = selectedDay.toString().split(" ").first;
        });
    ...
    return ListView(
      children: [
        ...
        Padding(
          ...
          child: Container(
            child: FutureBuilder(
              future: BudgetJsonService().getBudgetHistoryByDate(_selectedDay),
              builder: (BuildContext buildContext,
                  AsyncSnapshot<List<Budget>> snapshot) {
                if (snapshot.hasData) {
                  List<Budget>? budgetHistory = snapshot.data;
                  return Column(
                    children: [
                      for (var budget in budgetHistory!)
                        GestureDetector(
                          onTap: () {
                            TransactionDisplayer().showDetailsDialog(
                                context, budget, budget.budgetType!);
                          },
                          child: Padding(
